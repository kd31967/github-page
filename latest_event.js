const sheetID_1 = '1U1ViFgXBLtUaE119fyyiJSwTpP2hmnHGRLoqNAtlCI0';
const base = `https:docs.google.com/spreadsheets/d/${sheetID_1}/gviz/tq?`;
const sheetName = 'LatestEvent';
...
function maker(json) {
    ...
    const keys1 = Object.keys(json);
        for (var i = 0; i < keys1.length; i++) {
            ...
            console.log(i + ' - ' + counter.latest_event  + '' + counter.pic);
        }
    ...
    Object.keys(json).forEach(function(k){
        console.log(k + ' - ' + json[k]);
         newObject=json[k]
         console.log("121" + ' - ' + newObject.latest_event  + '' + newObject.pic);
         
		 //console.log(nums.includes(8));
		 if(!data.includes(newObject.latest_event))
		 {
			 data.push(newObject.latest_event)
			const li1 = document.createElement('li');
			li1.setAttribute("class","list ");
			li1.setAttribute("data-filter",newObject.latest_event);
			li1.textContent = newObject.latest_event;
		...
		div1.setAttribute("data-item",newObject.latest_event);
		...
		img.setAttribute("alt",newObject.latest_event);
